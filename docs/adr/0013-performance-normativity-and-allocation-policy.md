...
pre-optimization baseline (`bench/BASELINE.md`) — a quiescent pump allocating
...
- Performance work is falsifiable: every optimization PR cites the baseline row
  ...
  `bench/BASELINE.md`, the running post-change record in
  `bench/PROFILE-default.md` (the first `PerformanceConformanceProfile`) —
